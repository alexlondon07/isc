...
class FileUpload {
	...
	*/
    ...
    public function remove_weird_char($str) {
	    ...
	    $realstr = str_replace("á","a",$str);
	    $realstr = str_replace("é","e",$realstr);
	    $realstr = str_replace("í","i",$realstr);
	    $realstr = str_replace("ó","o",$realstr);
	    $realstr = str_replace("ú","u",$realstr);
	    return $realstr;
    }
	...
	*/
	...
	//Se establece el Tama�o maximo permitido para cargar un archivo
	$fileMaxSize = $maxSizeMB * $this->MB_BYTE; 
	//Se establecen las Extensiones v�lidas de archivos para cargar
	...
	if ($isFile) {
		...
		if($moved) { 
		    ...
		    $code = 11;
		    $content = " move_uploaded_file (".$fileTmpName.", ".$uploadPath.".".$safe_filename.") Se ha producido un error al cargar el archivo. Codigo de error: ".$fileError." , ubicacion: ".$uploadPath.$safe_filename."  #Esto probablemente se debe a que necesita asignar permisos de lectura-escritura a la carpeta de destino: ".$uploadPath;
		}
	    } else {
		$content = "El archivo *" .$fileNameUser. "* no se pudo cargar";
		if ($fileSize >= $fileMaxSize) {
		    $code = 12;
		    $content = $content." debido a que su tama�o es Mayor que " .$fileMaxSize/$this->MB_BYTE. " MB."; 	
		}
		if (!preg_match($rEFileTypes, strrchr($safe_filename, '.'))) {
		    $code = 13;
		    $content = $content." debido a que la extensi�n del archivo es incorrecto. S�lo se permite cargar archivos con las siguientes extensiones: ". $fileValidType .".";
		}
	    }
	} else {
	    $code = 14;
