...
class ControllerCustomer {
    ...
    private function clidelete() {
        //Se pregunta si el cliente no tiene asociado un contacto
        $q = "SELECT * FROM mzt_usuario WHERE usr_contacto='si' AND mzt_cliente_cli_id = $this->id ";
        $con = mysql_query($q, $this->conexion) or die(mysql_error() . "***ERROR: " . $q);
        $resultado = mysql_num_rows($con);
        if ($resultado == 0) { //Es que no había ningun contacto asociado a esta id de cliente
            ...
            $arrjson = $this->UTILITY-> error_contact_asocied();  
            $this->response = ($arrjson);
        }
    }
